...
def _make_flow(hass, source="user"):
    """Instantiate a flow with a mutable context dict."""
    flow = CoverExtenderConfigFlow()
    flow.hass = hass
    flow.context = {"source": source}  # mappingproxy évité — dict mutable
    return flow
...
class TestConfigFlowUserStep:
    ...
    async def test_no_input_returns_form(self, hass):
        result = await _make_flow(hass).async_step_user()
        assert result["type"] == "form"
        assert result["errors"] == {}

    async def test_file_not_found_error(self, hass):
        result = await _make_flow(hass).async_step_user({"source": "/nonexistent/file.yaml"})
        assert result["type"] == "form"
    ...
    async def test_yaml_invalid_error(self, hass, tmp_path):
        ...
        bad.write_text("[\n", encoding="utf-8")
        result = await _make_flow(hass).async_step_user({"source": str(bad)})
        assert result["type"] == "form"
    ...
    async def test_no_covers_found_error(self, hass, tmp_path):
        ...
        result = await _make_flow(hass).async_step_user({"source": str(f)})
        assert result["type"] == "form"
