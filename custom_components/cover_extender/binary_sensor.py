...
Exposes sun_facing and enable_auto_shade as binary_sensor entities,
controlled by the value_as_sensor: section of the YAML config file.

  value_as_sensor:
    sun_facing: true          →  binary_sensor.<cover>_sun_facing
    enable_auto_shade: true   →  binary_sensor.<cover>_enable_auto_shade

Both default to false — no binary_sensors are created unless explicitly enabled.
"""
...
async def async_setup_entry(
    ...
    value_as_sensor: dict = hass.data.get(DOMAIN, {}).get(DATA_VALUE_AS_SENSOR, {})

    sun_facing_entities, auto_shade_entities = _build_entities(profiles, value_as_sensor)
    ...
    all_entities = sun_facing_entities + auto_shade_entities
    _LOGGER.info(
        "cover_extender: created %d binary_sensor entities (%d sun_facing, %d enable_auto_shade)",
        len(all_entities), len(sun_facing_entities), len(auto_shade_entities),
    )
    ...
    def _handle_platform_reload() -> None:
        ...
        new_vas: dict = hass.data[DOMAIN].get(DATA_VALUE_AS_SENSOR, {})

        new_sf, new_as = _build_entities(new_profiles, new_vas)
        ...
        for cover_id in removed_as:
            cover_name = cover_id.split(".")[1]
            eid = registry.async_get_entity_id("binary_sensor", DOMAIN, f"{DOMAIN}_binary_sensor_{cover_name}_enable_auto_shade")
            if eid:
                registry.async_remove(eid)
        ...
        if added_sf or removed_sf or added_as or removed_as:
            _LOGGER.info(
                "cover_extender binary_sensor reload: +%d/-%d sun_facing, +%d/-%d enable_auto_shade",
                len(added_sf), len(removed_sf), len(added_as), len(removed_as),
            )
...
def _build_entities(
    profiles: dict,
    value_as_sensor: dict,
) -> tuple[list, list]:
    """Build entity lists according to value_as_sensor flags and profiles."""
    expose_sf = value_as_sensor.get("sun_facing", False)
    expose_as = value_as_sensor.get("enable_auto_shade", False)
...
class CoverEnableAutoShadeBinarySensor(BinarySensorEntity):
    """Exposes the enable_auto_shade config value as a binary_sensor.
    ...
    def __init__(self, cover_entity_id: str, enable_auto_shade: bool) -> None:
        self._cover_entity_id = cover_entity_id
        cover_name = cover_entity_id.split(".")[1]
        friendly = cover_name.replace("_", " ").title()

        self.entity_id = f"binary_sensor.{cover_name}_enable_auto_shade"
        self._attr_unique_id = f"{DOMAIN}_binary_sensor_{cover_name}_enable_auto_shade"
        self._attr_has_entity_name = True
        self._attr_translation_key = "enable_auto_shade"
        self._attr_is_on = enable_auto_shade
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, cover_entity_id)},
            name=friendly,
            entry_type=DeviceEntryType.SERVICE,
