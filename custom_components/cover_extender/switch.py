...
async def async_setup_entry(
    ...
    all_entities = lock_entities + auto_shade_entities
    _LOGGER.info(
        "cover_extender: created %d switch entities (%d auto_shade)",
        len(all_entities), len(auto_shade_entities),
    )
    ...
    def _handle_platform_reload() -> None:
        ...
        if added_lock or removed_lock or added_auto_shade or removed_auto_shade:
            _LOGGER.info(
                "cover_extender switch reload: +%d/-%d locks, +%d/-%d auto_shade",
                len(added_lock), len(removed_lock), len(added_auto_shade), len(removed_auto_shade),
            )
